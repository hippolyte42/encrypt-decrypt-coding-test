    ...
    IDropERC721
{
        ...
        for (uint256 i = 0; i < baseURIIndices.length; i += 1) {
            if (_tokenId < baseURIIndices[i]) {
                if (encryptedBaseURI[baseURIIndices[i]].length != 0) {
                    return
                        string(
                            abi.encodePacked(baseURI[baseURIIndices[i]], "0")
                        );
                } else {
                    return
                        string(
        ...
        if (_resetClaimEligibility) {
            ...
            if (existingPhaseCount > _phases.length) {
                for (uint256 i = _phases.length; i < existingPhaseCount; i++) {
                    ...
                    delete claimCondition.limitMerkleProofClaim[
                        newStartIndex + i
                    ];
                }
            }
    ...
    function transferClaimedTokens(
        ...
        claimCondition.limitLastClaimTimestamp[_conditionId][
            _msgSender()
        ] = block.timestamp;
    ...
    function verifyClaim(
        ...
        ClaimCondition memory currentClaimPhase = claimCondition.phases[
            _conditionId
        ];
        ...
        require(
            nextTokenIdToClaim + _quantity <= nextTokenIdToMint,
            "not enough minted tokens."
        );
        require(
            maxTotalSupply == 0 ||
                nextTokenIdToClaim + _quantity <= maxTotalSupply,
        ...
        ) = getClaimTimestamp(_conditionId, _claimer);
        require(
    ...
    function verifyClaimMerkleProof(
        ...
        if (currentClaimPhase.merkleRoot != bytes32(0)) {
            ...
            require(
                !claimCondition.limitMerkleProofClaim[_conditionId].get(
                    merkleProofIndex
                ),
                "proof claimed."
            );
    ...
    function getActiveClaimConditionId() public view returns (uint256) {
        for (
            ...
            if (
                block.timestamp >= claimCondition.phases[i - 1].startTimestamp
            ) {
                return i - 1;
            }
        }
    ...
    {
        lastClaimTimestamp = claimCondition.limitLastClaimTimestamp[
            _conditionId
        ][_claimer];
    ...
    function setMaxWalletClaimCount(uint256 _count)
        external
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
    ...
    {
        require(
            _maxTotalSupply < nextTokenIdToMint,
            "existing > desired max supply"
        );
